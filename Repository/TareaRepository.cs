...
public class TareaRepository : ITareaRepository{
    ...
    public void UpdateTarea(int idTarea, Tarea tarea){
        using(SQLiteConnection connection = new SQLiteConnection(cadenaConexion)){
            ...
            command.CommandText = @"UPDATE Tablero SET id_tablero = @id_tablero, nombre = @nombre, estado = @estado, descripcion = @descripcion, color = @color WHERE id = @idTarea;";
            command.Parameters.Add(new SQLiteParameter("@id_tablero", tarea.IdTablero));
            command.Parameters.Add(new SQLiteParameter("@nombre", tarea.Nombre));
            command.Parameters.Add(new SQLiteParameter("@estado", tarea.Estado));
            command.Parameters.Add(new SQLiteParameter("@descripcion", tarea.Descripcion));
    ...
    public void UpdateEstadoTarea(int idTarea, EstadoTarea estado){
        using(SQLiteConnection connection = new SQLiteConnection(cadenaConexion)){
            SQLiteCommand command = connection.CreateCommand();
            command.CommandText = @"UPDATE Tablero SET estado = @estado WHERE id = @idTarea;";
